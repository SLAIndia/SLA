...
public class Common {
	...
	public static java.sql.Timestamp getFormatedSqlDateTimestamp(String strDate) {
		...
		try {
			if (strDate != null && !strDate.trim().equals("")) {
				...
				sqlDate = java.sql.Timestamp.valueOf(sdf.format(dtUtilDate));
				// System.out.println("Date while parsing GIVEN DATE  :  "+strDate
				// +"   FORMATED DATE  :  "+sqlDate);
			} else {
				System.out
	...
	public static LinkedHashMap<String, String> userValidation(String signature,
			Integer id) {
		...
		try {
			...
			if (loginMap.containsKey(id)) {
				if (loginMap.get(id).toString()
						.equals(signature)) {
					
					resultMap.put("status", "1");
					resultMap.put("msg", MessageConstants.msg_1);
				} else {
					logger.info("user validation : " + resultMap.get("message"));
					resultMap.put("status", "9");
					resultMap.put("msg", MessageConstants.msg_9);
				}
			}else{
				resultMap.put("msg", MessageConstants.msg_8);
			...
			resultMap.put("msg", MessageConstants.msg_10);
		}
		return resultMap;
	}
		public static String addSignature(UserEntity objEntity) {
			double random = 0.0;
			...
			if(loginMap.containsKey(objEntity.getId())){
				return loginMap.get(objEntity.getId()).toString();
			}
		...
		public static Timestamp parseStringDate(String strDate){
			java.util.Date dtUtilDate = new java.util.Date();
